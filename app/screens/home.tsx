...
//   <CalendarProvider
//     date={selected}              // controlled current date
//     onDateChanged={setSelected} // callback when date changes
//   >
//     <WeekCalendar
//       current={selected}
...
//       }}
//       firstDay={1}
//       style={{ width: '100%' }} // Ensure calendar itself is full width
//     />
//   </CalendarProvider>
// </View>
...
console.log("Filtered Habits for this day:", filteredHabits.map(h => h.name))
  ...
  const dayProgressData = useMemo(() => {
    ...
    const dayHabits = filteredHabits.filter((habit) =>
      habit.frequency?.includes(dayLabel),
    )
    ...
    selected,
    filteredHabits
      .map((h) => `${h.name}-${h.current}-${h.target}-${h.frequency}`)
      .join(","),
  ])
  ...
  return (
    <>
      ...
      >
        ...
        <Toggle variant="checkbox" inputOuterStyle={$checkboxInput} value={task.current >= task.target} />
      </TouchableOpacity>
